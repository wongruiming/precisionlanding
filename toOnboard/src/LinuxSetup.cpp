...
ackReturnData takeControl(CoreAPI* api)
{
  ...
  switch (takeControlData.ack)
  {
    case ACK_SETCONTROL_NEED_MODE_F:
      std::cout << "Failed to obtain control. Your RC mode switch is not in mode F. (Is the RC connected and paired?)" << std::endl;
    ...
    case ACK_SETCONTROL_IOC:
      std::cout << "The aircraft is in IOC mode. Cannot obtain control. Go to DJI GO and stop all intelligent flight modes before trying this." << std::endl;
      takeControlData.status = -1;
      return takeControlData;
      break;
    default:
